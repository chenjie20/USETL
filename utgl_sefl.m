function [F, iter, obj_values] = utgl_sefl(Ln, Hn, alpha, beta)
...
%         max_eigen = eigs(L_star, 1 ,'la');
        ...
        while iter2 < max_iter2
            iter2 = iter2 + 1;   
            M = (max_eigen * eye(size(L_star))-L_star) * Hn_updated(:, :, nv_idx) + beta * F * P(:, : , nv_idx)';
            [Um, ~, Vm] = svd(M, 'econ');
